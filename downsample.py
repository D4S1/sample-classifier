...
def evaluate_downsampling(training_file: str, testing_file: str, ground_truth_file: str, output_prefix: str, ratios: List[float], downsample: str):
    """
    ...
    :param downsample: Specifies which dataset to downsample ('training', 'testing', or 'both').
    """
    ...
    for ratio in ratios:
        ...
        train_data = utils.load_ref(training_file)

        train_downsampled = training_file
        test_downsampled = testing_file

        # Downsample training data if specified
        if downsample in ('training', 'both'):
            train_downsampled = f"{output_prefix}_train_{int(ratio * 100)}.fasta.gz"
            downsample_fasta(training_file, train_downsampled, ratio)

        # Downsample testing data if specified
        if downsample in ('testing', 'both'):
...
def main():
    ...
    parser.add_argument("--ratios", type=float, nargs='+', default=[0.1, 0.2, 0.5, 0.75], help="List of downsampling ratios.")
    parser.add_argument("--downsample", type=str, choices=['training', 'testing', 'both'], default='both',
                        help="Specify whether to downsample 'training', 'testing', or 'both' datasets.")
